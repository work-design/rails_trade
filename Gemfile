source 'https://rubygems.org'
git_source(:github) { |repo| "https://github.com/#{repo}.git" }
gemspec

gem 'puma'
gem 'rails', '>= 5.2.0'

gem 'mysql2'
gem 'sqlite3'

gem 'pry-rails'

gem 'kaminari', github: 'amatsuda/kaminari'
gem 'sprockets', github: 'rails/sprockets'

# for test
gem 'factory_bot_rails'
gem 'capybara'

# dependence engines
gem 'default_where', github: 'qinmingyuan/default_where', branch: 'master'
gem 'default_form', github: 'qinmingyuan/default_form', branch: 'master'

# engines
gem 'rails_com', github: 'yougexiangfa/rails_com', branch: 'master'
gem 'rails_data', github: 'yougexiangfa/rails_data', branch: 'master'
gem 'rails_role', github: 'yougexiangfa/rails_role', branch: 'master'
gem 'rails_audit', github: 'yougexiangfa/rails_audit', branch: 'master'
