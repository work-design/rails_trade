class TheTradeAdmin::PaymentsController < TheTradeAdmin::TheTradeController
  ...
  def index
    @payments = Payment.default_where(params.permit(:type, :state, :'payment_orders.state', :id)).default_where(params.fetch(:q, {}).permit(:'buyer_name-like', :buyer_identifier, :buyer_bank, :payment_uuid, :'orders.uuid')).order(id: :desc).page(params[:page])
  end
