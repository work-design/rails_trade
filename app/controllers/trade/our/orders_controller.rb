module Trade
  class Our::OrdersController < My::OrdersController
    ...
    def index
      ...
      @orders = current_client.organ.orders.includes(:member_organ).default_where(q_params).order(id: :desc).page(params[:page]).per(params[:per])
    end
    ...
    private
    def set_order
      @order = current_client.organ.orders.find(params[:id])
    end

    def set_new_order
      @order = current_client.organ.orders.build(order_params)
    end
